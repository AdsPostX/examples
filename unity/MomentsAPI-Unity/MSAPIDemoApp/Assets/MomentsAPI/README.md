...
- `HandlePositiveAction(Offer, int, int, Action<bool>)` - Handle accept button
- `HandleNegativeAction(Offer, int, int, Action<bool>)` - Handle decline button
- `HandleCloseAction(Offer)` - Handle close button
- `HandleDisplayTracking(Offer)` - Send display tracking beacons
- `SendTrackingRequest(string)` - Send custom tracking request
...
{
    public Dictionary<string, object>? popup;
    public Dictionary<string, object>? offerText;
...
For issues, questions, or feature requests, please contact the Moments API support team.
